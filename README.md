...
- Cognito: AWS Cognito User Import Job
...
- そのため AWS 認証情報を入れる場合、通常は `S3_ENDPOINT` を空にして AWS S3 を使ってください
...
- CSV 内重複チェック
- 既存 username / 実行中ジョブとの突合による `NEW` / `ERROR` 判定
...
アプリ側の `.env` だけでは足りません。AWS 側で少なくとも以下を用意してください。
...
このアプリは Cognito の `CreateUserImportJob` / `StartUserImportJob` / `DescribeUserImportJob` を使います。
